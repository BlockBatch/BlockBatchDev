...
const Settings: FC = () => {
    ...
    if (confirm(`Are you sure you want to rotate the API key "${keyName}"? This will generate a new key and invalidate the existing one.`)) {
      ...
      setApiKeys(apiKeys.map(k => {
        if (k.name === keyName) {
          return {
            ...k,
            key: `${keyName.substring(0, 2).toLowerCase()}_${Math.random().toString(36).substring(2, 6)}****************${Math.random().toString(36).substring(2, 5)}`,
  ...
  const addNewApiKey = () => {
    ...
    if (newKeyName) {
      const newKey: ApiKey = {
        name: newKeyName,
        key: `${newKeyName.substring(0, 2).toLowerCase()}_${Math.random().toString(36).substring(2, 6)}****************${Math.random().toString(36).substring(2, 5)}`,
        created: new Date().toISOString().split('T')[0],
        lastUsed: "Never",
                      ...
                      <tr key={apiKey.name} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 flex items-center space-x-2">
                          <span className="font-mono">
                            {visibleKeys[apiKey.name] ? apiKey.key.replace('****************', Math.random().toString(36).substring(2, 18)) : apiKey.key}
                          </span>
                          <button 
